...
<script>
import { getLingfannaoDomain, getLingfannaoDomainRecord } from '@/api/domain'
...
export default {
  ...
  methods: {
    getList() {
      getLingfannaoDomain(this.queryList).then(response => {
    ...
    showRecords(domain_name) {
      ...
      getLingfannaoDomainRecord(this.recordQueryList).then(response => {
